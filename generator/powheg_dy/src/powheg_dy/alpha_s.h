...
namespace powheg_dy
{
    inline double alphaS0(const Config& config, double qSq, int nF)
    {
        return 1.0 / (config.beta0(nF) * std::log(qSq / config.LAMBDA_MSB_5_SQ));
    }

    inline double alphaS0customLambda(const Config& config, double qSq, int nF, double lambdaSq)
    {
        return 1.0 / (config.beta0(nF) * std::log(qSq / lambdaSq));
    }

    inline int activeFlavours(const Config& config, double qSq)
    {
    ...
    inline double betaPrimePowheg(int nf)
    {
        return (153.0 - 19.0 * nf) / (2.0 * PI * (33.0 - 2.0 * nf));
    }

    inline double alphaRawPowheg(const Config& config, double qSq, int nF)
    {
    ...
    inline double alphaSPowheg(const Config& config, double qSq) 
    {
    ...
    inline double alphaSCMW(const Config& config, double qSq) 
    {
        const int nF = activeFlavours(config, qSq);
        const double alphaS = alphaSPowheg(config, qSq);
